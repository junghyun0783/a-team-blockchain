...
<body>
	...
	$(document).ready(function(){
		...
		function showList(page){
			...
			replyService.getList({bno:bnoVal, page:page||1}, function(replyCount, data){ // replyCount, data 매개변수 2개로 수정  
				...
				for(let i = 0; i < len; i++){
					...
					str += "			<button class='addRepBtn'>답글</button>";
					str += "			<button class='modifRepBtn'>수정</button>";
					str += "		</div>";
					str += "	</div>";
					str += "</div>";
				}
		...
		$(".reply_list_container").on("click", "button.addRepBtn", function(e){
			...
			str 	+= "		<input type='text' placeholder='작성자' />";
			str 	+= "		<button class='addRepSubmit'>답글저장</button>";
			str 	+= "	</div>";
			str 	+= "</div>";
		...
		$(".reply_list_container").on("click", "button.modifRepBtn", function(e){
			...
			*/
			...
			str 	+= "<div class='reply_btns'>";
			str 	+= "	<button class='modifRepSubmit'>수정확인</button>";
			str 	+= "	<button class='deleteRep'>삭제</button>";
			str 	+= "	<button class='modifCancel'>취소</button>";
			str 	+= "</div>";
